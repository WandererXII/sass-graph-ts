...
import { fixture, graph } from './util.js';
  ...
  describe('parseFile', function () {
    describe('with a simple graph', function () {
      it('should return a graph', function () {
        graph()
          ...
          .assertAncestors(path.join('nested', '_d.scss'), ['_c.scss', 'b.scss', 'index.scss'])
          .assertAncestors('_c.scss', ['b.scss', 'index.scss'])
          .assertAncestors('b.scss', ['index.scss'])
    ...
    describe('with a simple graph in indented syntax', function () {
      it('should return a graph', function () {
        graph()
          .indented()
          .fromFixtureFile('indented-syntax')
          .assertDecendents([
            'b.sass',
            '_c.sass',
          ...
          .assertAncestors(path.join('nested', '_d.sass'), ['_c.sass', 'b.sass', 'index.sass'])
          .assertAncestors('_c.sass', ['b.sass', 'index.sass'])
          .assertAncestors('b.sass', ['index.sass'])
    ...
    describe('with a graph with loadPaths', function () {
      it('should return a graph', function () {
        ...
        graph(opts)
          ...
          .assertAncestors(path.join(includeFolder, '_b.scss'), ['index.scss'])
          .assertAncestors('index.scss', [])
      ...
      it('should prioritize cwd', function () {
        ...
        graph(opts)
          .fromFixtureFile('load-path-cwd')
          .assertDecendents(['_b.scss', path.join(includeFolder, '_c.scss')])
          .assertAncestors(path.join(includeFolder, '_c.scss'), ['_b.scss', 'index.scss'])
          .assertAncestors('_b.scss', ['index.scss'])
          .assertAncestors('index.scss', [])
    ...
    describe('with a folder that has an extension', function () {
      it('should return a graph', function () {
        ...
        graph()
          .fromFixtureFile('folder-with-extension')
    ...
    describe('with exclusion pattern', function () {
      it('should exclude all files matching the regular expression', function () {
        graph({ exclude: /exclude-/ })
          ...
          .assertDecendents(['dont-exclude.scss']);
      });
    });
  });
});
